...
import { mockDustbins, mockWasteReports, mockCleanupDrives } from '../../data/mockData';
...
const MapComponent: React.FC<MapComponentProps> = ({
  ...
  const toggleDustbinStatus = (dustbinId: string) => {
    ...
    setDustbins(prev => prev.map(dustbin => {
  ...
  return (
    <div style={{ height }} className="rounded-lg overflow-hidden shadow-lg">
      ...
      >
        ...
        {/* Dustbin markers */}
        {showLayers.dustbins && dustbins.map(dustbin => (
          <Marker
            key={dustbin.id}
            position={[dustbin.latitude, dustbin.longitude]}
            icon={dustbin.status === 'empty' ? dustbinEmptyIcon : dustbinFilledIcon}
          >
            <Popup>
              <div className="p-2 min-w-[200px]">
                ...
                <p className="text-sm text-gray-600 mb-2">{dustbin.address}</p>
                ...
                <p className="text-xs text-gray-500">
                  Last updated: {new Date(dustbin.lastUpdated).toLocaleDateString()}
                </p>
              </div>
            </Popup>
          </Marker>
        ...
        {showLayers.wasteReports && mockWasteReports.map(report => (
          <Marker
            key={report.id}
        ...
        {showLayers.cleanupDrives && mockCleanupDrives.map(drive => (
          <Marker
            key={drive.id}
          ...
          >
            <Popup>
              <div className="p-2 min-w-[200px]">
                ...
                <p className="text-sm font-medium mb-1">{drive.communityName}</p>
